...
<script>
import basicFrame from '@/components/layout/basic-frame/main'
...
export default {
  ...
  methods: {
    ...
    // 加载浏览器窗口变化自适应
    resize() {
      window.onresize = () => { this.iframeInit() }
    },
    // 加载组件
    load() {
      this.show()
    ...
    iframeInit() {
      ...
      if (!iframe) return
      iframe.style.height = `${clientHeight + 7}px`
      if (iframe.attachEvent) {
        iframe.attachEvent("onload", () => { this.hide() })
      } else {
        iframe.onload = () => { this.hide() }
      }
    },
    getUrlPath: function() {
      //获取 iframe src 路径
