<template>
  <div class="avue-tags" v-if="showTag" @click="contextmenuFlag=false">
    <!-- tag盒子 -->
    ...
    <div class="avue-tags__box" :class="{'avue-tags__box--close':!website.isFirstPage}">
      ...
      <el-dropdown class="avue-tags__menu">
        <el-button type="primary" size="mini"> {{$t('tagsView.menu')}} <i class="el-icon-arrow-down el-icon--right"></i> </el-button>
...
export default {
  ...
  computed: {
    ...mapGetters(['tag', 'tagHome', 'tagList', 'website']),
    ...mapState({ showTag: state => state.comm.showTag }),
  ...
  methods: {
    ...
    openTag(item) {
      ...
      this.$router.push({
        path: this.$router.$routerPlugin.getPath({ name: tag.label, src: tag.value }, tag.meta),
        query: tag.query
      })
    },
    closeOthersTags() {
    ...
    closeAllTags() {
      ...
      this.$router.push({
        path: this.$router.$routerPlugin.getPath({ src: this.tagHome.value }),
        query: this.tagHome.query
      })
    }
  }
}
