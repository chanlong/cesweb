<template>
  <!-- 添加或修改菜单对话框 -->
  <el-dialog :title="!form.menuId ? '新增': '修改'" :visible.sync="visible" :modal-append-to-body="false">
    <el-form ref="dataForm" :model="form" :rules="rules" label-width="80px">
      ...
      <el-form-item label="图标" prop="icon" v-if="form.type !== '1'">
        <avue-input-icon v-model="form.icon" placeholder="请选择图标" :icon-list="iconList"></avue-input-icon>
      </el-form-item>
...
const GR = GlobalRequest.server('/system/menu')
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          if (this.form.menuId) {
            GR.update(this.form).then(response => {
              this.$message.success('修改成功')
              this.visible = false
              this.$emit('refreshDataList')
            })
          } else {
            GR.create(this.form).then(response => {
