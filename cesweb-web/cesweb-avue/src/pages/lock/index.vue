...
export default {
  ...
  methods: {
    handleLogout() {
      this.$confirm("是否退出系统, 是否继续?", "提示", { confirmButtonText: "确定", cancelButtonText: "取消", type: "warning" }).then(() => {
    ...
    handleLogin() {
      if (this.passwd != this.lockPasswd) {
        this.passwd = ""
        this.$message({ message: "解锁密码错误,请重新输入", type: "error" })
        this.passwdError = true
        setTimeout(() => { this.passwdError = false }, 1000)
        return
      }
      this.pass = true
      setTimeout(() => {
